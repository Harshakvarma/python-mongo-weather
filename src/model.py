...
class DeviceModel:
    ...
    def insert(self, device_id, desc, type, manufacturer):
        self._latest_error = ''
        if (self.current_user != 'admin' or (self.current_user != 'user_1' and device_id == 'DT002')):
            self._latest_error = f'Insert failed, Admin access required!'
            return -1
...
class DailyReportModel:
    WEATHER_DATA_COLLECTION = 'daily_reports'
    ...
    # This first checks if a data item already exists at a particular timestamp for a device id. If it does, it populates latest_error and returns -1.
    # If it doesn't already exist, it'll insert a new document and return the same to the caller
    def insert(self, device_id, avg_value, min_value, max_value, timestamp):
        self._latest_error = ''
        if (self.current_user != 'admin'):
            self._latest_error = f'Insert failed, Admin access required!'
            return -1
    ...
    # This first checks is to inset multiple records
    # def insertMultiple(self, device_id, avg_value, min_value, max_value, timestamp):
    #     self._latest_error = ''
    #     if (self.current_user != 'admin'):
    #         self._latest_error = f'Insert failed, Admin access required!'
    #         return -1
